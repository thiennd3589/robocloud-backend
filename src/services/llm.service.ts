...
export class LlmService {
  ...
  private genCodePrompt = `
    Dựa vào những thông tin ở trên, hãy tạo đoạn mã cho robot arduino code C/C++ phù hợp. Phản hồi chỉ sinh mã code arduino C/C++ và KHÔNG kèm giải thích.
    ...
    -  Nút nhấn trái với chân 7, NẾU SỬ DỤNG sẽ được khai báo như sau:
    #define BUTTON_LEFT 7 // Khi lập trình, nút được xác định là đang được nhấn khi digitalRead(BUTTON_LEFT) == HIGH

    -  Nút nhấn Phải với chân 8, NẾU SỬ DỤNG sẽ được khai báo như sau:
    #define BUTTON_RIGHT 8 // Khi lập trình, nút được xác định là đang được nhấn khi digitalRead(BUTTON_RIGHT) == HIGH
  ...
  private systemInstructions = `
    Bạn là một chuyên gia về lập trình C trên Kit Arduino và có khả năng sư phạm cho học sinh. Bạn đang lập trình robot arduino có động cơ di chuyển và màn hình OLED.
